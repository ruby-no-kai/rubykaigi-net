    ...
    extraInitContainers: [
      {
        ...
        command: ['sh', '-c'],
        args: [
          'wget -O /tmp/plugin.zip https://github.com/hanazuki/grafana-weathermap/releases/download/v%(version)s/hanazuki-weathermap-panel-%(version)s.zip && unzip /tmp/plugin.zip -d /var/lib/grafana/plugins/' % { version: '0.1.12' },
        ],
        volumeMounts: [
          {
            name: 'storage',
